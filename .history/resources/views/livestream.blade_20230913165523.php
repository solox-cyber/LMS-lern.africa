                ...
                <div id="accordion" class="accordion mb-0">
                    ...
                    <div class="card shadow-xss mb-0">
                        <div class="card-header bg-greylight theme-dark-bg" id="heading{{ $syllabusTitle->id }}">
                            <h5 class="mb-0">
                                <button class="btn font-xsss fw-600 btn-link" data-toggle="collapse" data-target="#collapse{{ $syllabusTitle->id }}" aria-expanded="false" aria-controls="collapse{{ $syllabusTitle->id }}">
                                    {{ $syllabusTitle->title }}
                                </button>
                            </h5>
                        </div>
                        <div id="collapse{{ $syllabusTitle->id }}" class="collapse p-3" aria-labelledby="heading{{ $syllabusTitle->id }}" data-parent="#accordion">
                            <div class="card-body">
                                <!-- Loop through subtitles for this syllabus title -->
                                @foreach ($syllabusTitle->coursesubtopic as $subtitle)
                                <div class="card shadow-xss mb-0">
                                    <div class="card-header bg-greylight theme-dark-bg" id="heading{{ $subtitle->id }}">
                                        <h5 class="mb-0">
                                            <button class="btn font-xsss fw-600 btn-link" data-toggle="collapse" data-target="#collapse{{ $subtitle->id }}" aria-expanded="false" aria-controls="collapse{{ $subtitle->id }}">
                                                {{ $subtitle->title }}
                                            </button>
                                        </h5>
                                    </div>
                                    ...
                                    <span class="bg-current btn-round-xs rounded-lg font-xssss text-white fw-600">tt</span>
                                    <span class="font-xssss fw-500 text-grey-500 ml-2">77</span>
                                    <span class="ml-auto font-xssss fw-500 text-grey-500">8787</span>
                                </div>
                                        </div>
                                    </div>
                                </div>
