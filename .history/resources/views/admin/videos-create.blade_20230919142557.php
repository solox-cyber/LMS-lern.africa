...
<div class="app-main flex-column flex-row-fluid" id="kt_app_main">
                            ...
                            <form action="{{ route('videos.store') }}" class="form mb-15" method="POST" id="kt_careers_form" enctype="multipart/form-data">
                                ...
                                <div class="row mb-5">
                                    ...
                                    <div class="col-md-12 fv-row">
                                        ...
                                        <select name="subtopic_id" id="subtopic_id" class="form-control form-control-solid">
                                            <!-- Options will be populated dynamically using JavaScript -->
                                ...
                                <!--end::Input group-->
                                    ...
                                    <div class="col-md-12 fv-row">
                                        <!--begin::Label-->
                                        <label class="required fs-5 fw-semibold mb-2">Enter Course Sub Topic</label>
                                        <!--end::Label-->
    ...
    $(document).ready(function() {
        ...
        $('#course_id').on('change', function() {
            ...
            $.ajax({
                url: '{{ route('fetch.course', ['courseId' => ':courseId']) }}'.replace(':courseId', courseId),
                type: 'GET',
        ...
        $('#syllabus_title_id').on('change', function() {
            // Get the selected syllabus ID
            var syllabusId = $(this).val();

            // Send an AJAX request to fetch course sub topics for the selected syllabus
            $.ajax({
                url: '{{ route('fetch.subtitle', ['syllabusId' => ':syllabusId']) }}'.replace(':syllabusId', syllabusId),
                type: 'GET',
                success: function(data) {
                    // Clear and update the course subtopic selection dropdown
                    $('#subtopic_id').empty();
                    ...
                    if (Object.keys(data).length > 0) {
                        // Populate the dropdown with retrieved data
                        $.each(data, function(key, value) {
                            $('#subtopic_id').append('<option value="' + key + '">' + value + '</option>');
                        });
                    } else {
                        // Handle the case when no course subtopics are available for the selected syllabus
                        $('#subtopic_id').append('<option value="">No titles available</option>');
                    }
                },
                error: function() {
                    // Handle errors if the request fails
                    console.error('Failed to fetch course sub topics.');
                }
            });
        });
        ...
        $('input[name="video"]').on('change', function() {
            ...
            if (file) {
                ...
                // Send an AJAX request to handle the video upload
                $.ajax({
                    url: '{{ route('videos.store') }}', // Update to your video upload route
                    type: 'POST',
                    data: formData,
                    contentType: false,
                    processData: false,
                    xhr: function() {
                        // Create an XMLHttpRequest object with upload progress tracking
                    ...
                    success: function(data) {
                        ...
                        uploadStatus.html('<div class="alert alert-success">Upload Complete</div>');
                    },
                    error: function() {
                        // Handle errors if the request fails
