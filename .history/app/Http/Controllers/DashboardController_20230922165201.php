...
class DashboardController extends Controller
{
    ...
    public function LiveStreambyid($courseid)
    {
        ...
        // Check if the user's service matches the courseid
        if ($user->service !== $courseid) {
            abort(403, 'Unauthorized access'); // Deny access if the user's service does not match the courseid
        }
    ...
    We use Auth::user() to get the authenticated user.
    We then check if the service of the authenticated user matches the provided courseid. If they do not match, we return a 403 forbidden response, indicating unauthorized access.
    If the user's service matches the courseid, we proceed to retrieve the course with that ID and display it.
    This way, only users whose service matches the provided courseid will have access to the livestream view, and others will be denied access.
    ...
    public function VideoStore(Request $request){
        ...
        return redirect()->route('videos.create')->with('success', 'Course Video created successfully.');
    }
